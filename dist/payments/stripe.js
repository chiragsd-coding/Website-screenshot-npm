...
if (config.STRIPE_SECRET_KEY) {
    stripe = new Stripe(config.STRIPE_SECRET_KEY, {
        apiVersion: '2024-04-10', // use a stable standard version compatible with the installed SDK
    });
}
export class StripeGateway {
    ...
    async createSubscription(customerId, tier) {
        try {
            ...
            const subscriptionId = session.subscription || `stripe_checkout_pending_${session.id}`;
            return {
                subscriptionId,
