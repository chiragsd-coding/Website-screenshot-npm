...
export function getSubscriptionByApiKey(apiKey: string): SubscriptionRecord | null {
  try {
...
export async function upgradeOrDowngradeSubscription(
  apiKey: string,
  newTier: 'pro' | 'business' | 'enterprise'
...
export async function cancelSubscription(apiKey: string): Promise<void> {
  const sub = getSubscriptionByApiKey(apiKey);
  if (!sub) {
