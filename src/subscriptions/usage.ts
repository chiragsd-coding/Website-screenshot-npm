...
export function getUsage(apiKey: string, month: string = getCurrentMonth()): number {
  try {
    const row = db.prepare('SELECT count FROM usage WHERE api_key = ? AND month = ?').get(apiKey, month) as { count: number } | undefined;
...
export function incrementUsage(apiKey: string, month: string = getCurrentMonth()): void {
  try {
    db.prepare(`
...
export function isLimitExceeded(apiKey: string, month: string = getCurrentMonth()): boolean {
  try {
