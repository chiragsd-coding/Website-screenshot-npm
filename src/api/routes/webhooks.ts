...
router.post('/', async (req: Request & { rawBody?: string }, res: Response) => {
  try {
    ...
    let localSub = db.prepare('SELECT * FROM subscriptions WHERE subscription_id = ?').get(subscriptionId) as any;

    if (!localSub && customerId) {
      localSub = db.prepare('SELECT * FROM subscriptions WHERE customer_id = ?').get(customerId) as any;
    }
    ...
    if (!localSub) {
      ...
      if (email) {
        db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `).run(
          `ps_${Math.random().toString(36).substring(2, 15)}`, // simple fallback key
          email,
          'pro', // assume pro fallback
          status || 'active',
          selectedGateway,
          customerId || null,
          subscriptionId
