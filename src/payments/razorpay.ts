...
export class RazorpayGateway implements PaymentGateway {
  ...
  private planCache: Record<string, string> = {};
  ...
  public async syncPlans(): Promise<void> {
    ...
    for (const tier of tiers) {
      try {
        if (this.planCache[tier]) continue;
        ...
        let rzPlan = plansList.items.find(
          (p: any) => p.item?.name === plan.name && p.item?.currency === 'INR'
        );
  ...
  public async handleWebhook(payload: any, signature: string): Promise<NormalizedWebhookEvent> {
    ...
    if (webhookSecret && signature) {
      ...
      const isValid = (Razorpay as any).validateWebhookSignature(rawBody, signature, webhookSecret);
      if (!isValid) {
