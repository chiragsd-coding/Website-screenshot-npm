...
import { BrowserManager, browserManager } from './browser.js';
...
describe('Screenshot Engine Unit Tests', () => {
  ...
  describe('Screenshot Engine Rendering Integration', () => {
    afterAll(async () => {
      await browserManager.shutdown();
    });

    it('should successfully capture a screenshot of https://example.com', async () => {
      ...
      expect(result.image.length).toBeGreaterThan(1000); // Verify it's a real non-empty image
      expect(result.contentType).toBe('image/png');
      expect(result.fromCache).toBe(false);
      expect(result.durationMs).toBeGreaterThan(0);

      // Save a copy of the integration test screenshot to disk
      await fs.writeFile('example-integration-test.png', result.image);
    }, 20000); // 20s timeout
  });
});
